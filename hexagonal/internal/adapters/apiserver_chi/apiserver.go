...
//
//	-ldflags "-X 'github.com/mobiletoly/gokatana-samples/hexagonal/internal/adapters/apiserver_chi.AppTagVersion=1.0.0'"
func init() {
...
func Start(ctx context.Context, uc *usecase.UseCases) *http.Server {
	...
	server := kathttp_std.Start(
		...
		func(mux *http.ServeMux) http.Handler {
			...
			handler := slogchi.NewWithConfig(logger, config)(mux)
			return handler
		})
	return server
}
